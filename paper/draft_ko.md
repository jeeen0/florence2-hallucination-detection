...
- COCO val2017 100장 (Florence-2-large-ft) 실험에서 hallucination rate를 3.77%→1.29%로 줄이고, unsupported 탐지 F1 0.80, grounding accuracy@0.5 94%를 달성한다. 50장 base-ft 보조 실험에서도 같은 경향(8.22%→2.90%)을 확인한다.
...
### 4.1 데이터셋
COCO val2017 (5,000장)에서 무작위 추출한 두 분할을 사용한다.
- **Pilot (base-ft)**: 시드 42로 50장 (CANDIDATE_IDS 풀에서 셔플).
- **Main (large-ft)**: 시드 7로 `instances_val2017.json` 의 전체 ID 풀에서 100장.

두 분할은 별도이며 GT는 동일하게 `instances_val2017.json` 의 80개 객체 카테고리이다.

### 4.2 구현 세부
PyTorch 2.5.1 + CUDA 12.1, NVIDIA RTX 3070. 모델은 `microsoft/Florence-2-base-ft` (약 230M parameters) 및 `microsoft/Florence-2-large-ft` (약 770M parameters)을 fp16으로 로드하여 inference만 수행한다. caption / OD prompt 모두 num_beams=3, do_sample=False. 본 연구는 추가 학습 없이 zero-shot inference만 사용한다.
...
표 1은 두 모델·두 분할에 대한 결과를 합쳐 정리한 것이다.

**표 1. Base-ft (50장) 와 Large-ft (100장) 의 hallucination 검출 결과**
...
두 모델 모두 검증을 통해 hallucination rate가 약 ⅔ 감소했다 (base-ft 8.22→2.90%, large-ft 3.77→1.29%). large-ft 가 더 적은 baseline hallucination을 가지지만, 본 검증 파이프라인은 더 큰 모델에서도 추가로 hallucination을 잡아낸다. 양쪽 모두 unsupported 탐지의 confusion matrix가 TP=4, FP=0, FN=2 로 동일한 형태를 보였으며 (TN은 67 vs 153), 이로 인해 F1 0.8000이 두 모델 사이에서 우연적으로 정확히 일치한다. precision=1.0 (over-flag 없음), recall=0.667. supported mention 중 GT bbox와 IoU≥0.5 매칭율은 base-ft 97.01%, large-ft 94.12% 로 두 모델 모두 라벨 매칭이 단순 어휘 일치를 넘어 공간적 일관성을 동반함을 보였다.
...
### 6.1 해석
검증으로 hallucination rate가 base-ft 8.22→2.90%, large-ft 3.77→1.29%로 모두 약 ⅔ 떨어진 것은 모델 규모와 실험 분할이 다름에도 일관된 신호이다. precision=1.0은 시스템이 *과도 기각하지 않음*을 의미하며, recall 0.667은 *동일 모델 self-verification 만으로는 잡지 못하는 hallucination이 남아 있음*을 의미한다. 검출된 hallucination은 모두 caption-과 OD-의 *어휘 불일치형* (예: caption "baseball" vs OD "baseball bat") 이었고, 놓친 hallucination은 caption과 OD가 같은 방향으로 hallucinate한 joint failure 형태였다.
...
본 연구는 Florence-2의 통합 multi-task 능력을 활용해 caption-grounding consistency를 검증하는 self-verification 파이프라인을 제안하였다. COCO val2017 100장에 대한 Florence-2-large-ft 실험에서 hallucination rate가 3.77%에서 1.29%로, 50장 base-ft 실험에서도 8.22%에서 2.90%로 일관되게 약 ⅔ 감소했으며, unsupported 탐지 F1은 두 모델 모두 0.80, grounding accuracy@0.5는 각각 94.12%·97.01%를 보였다. 동일 모델 self-verification의 한계가 정성 분석에서 명확히 드러났고, 이 한계는 검증 모델 분리·정밀한 visual evidence·관계 hallucination 확장 등 명확한 후속 연구 방향으로 이어진다.
