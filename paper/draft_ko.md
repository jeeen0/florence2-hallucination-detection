...
본 연구는 통합 비전-언어 모델 Florence-2[1]의 captioning과 object detection 기능을 결합한 caption-detection consistency 검증 파이프라인을 구현하고, 그 효과와 한계를 실험적으로 분석한다. 파이프라인은 (i) `<CAPTION>` 결과에서 객체 mention을 추출하고, (ii) 동일 이미지의 `<OD>` 결과와 정규화된 라벨로 매칭하여 supported / unsupported로 판정하며, (iii) 평가 단계에서만 COCO GT와 비교하여 실제 hallucination 여부를 산정한다. COCO val2017 500장 (mentions 732건) 에서 Florence-2-large-ft는 baseline hallucination rate를 2.46%에서 검증 후 0.70%로 감소시켰으며 (95% CI 둘 사이 겹침 없음), unsupported 탐지 F1 = 0.812, supported & GT-present subset에 대한 grounding accuracy@0.5 = 95.65% (n=713) 를 달성했다. 외부 POPE benchmark[4] 질문을 객체 존재 판정 문제로 변환하여 평가한 결과, 본 OD-based verification baseline은 random / popular / adversarial 세 split에서 89.97~91.34% 정확도를 보였으며 이는 LLaVA-1.5-7B / InstructBLIP의 reported 값보다 높다 (단 detector 기반 판정이므로 직접적인 모델 우열 비교가 아닌 visual evidence 기반 검증의 가능성을 보여주는 참고 결과로 해석한다). 또한 동의어 매핑 정책 ablation을 통해 generic-to-specific 매핑이 baseline hallucination rate를 약 80% 부풀릴 수 있음을 정량 입증한다. Cross-model 분석 (BLIP-large caption + Florence-2 OD) 에서는 captioner를 바꾸어도 검증 recall이 0.571로 동일하게 관찰되어, 본 실험 범위에서 OD recall이 주요 병목일 가능성을 시사한다.
...
3. COCO val2017 500장 실험 (732 mentions, 95% bootstrap CI 포함) 에서 baseline 2.46% → verified 0.70% 의 통계적으로 유의한 감소, F1 = 0.812.
...
| Unsupported P / R / F1 | TP = 예측 unsupp AND canonical ∉ GT ; FP = 예측 unsupp AND ∈ GT ; FN = 예측 sup AND ∉ GT ; TN = 예측 sup AND ∈ GT |
...
| Method | Mentions | Baseline Hall% | Verified Hall% | F1 (Unsupp) | Grd Acc@0.5 (n=713) |
| --- | --: | --: | --: | --: | --: |
| `<CAPTION>` baseline | 732 | 2.46 [1.50, 3.55] | – | – | – |
| **Ours (verified)** | **718** | – | **0.70 [0.14, 1.39]** | **0.812 [0.636, 0.941]** | **95.65** |

검증을 통해 hallucination rate 가 통계적으로 유의하게 감소 (CI 겹침 없음). Confusion matrix: TP=13, FP=1, FN=5, TN=713 → Precision=0.929, Recall=0.722.

`<CAPTION>` 은 짧고 보수적인 설명을 생성하기 때문에 baseline hallucination rate가 2.46%로 낮다. 그러나 5.2 절에서 보듯 `<DETAILED_CAPTION>` 에서는 mention 수가 53% 증가하고 hallucination rate 도 6.07% 로 상승하여, caption 이 자세해질수록 object hallucination 문제가 더 뚜렷하게 나타난다.

**표 1b. 제거(removal) 통계 — 본 시스템이 "단순히 많이 지운 것" 이 아님을 보이는 분석**

| 항목 | 값 |
| --- | --: |
| Total mentions | 732 |
| Removed as unsupported | 14 |
| Correctly removed (TP, hallucination) | 13 |
| Incorrectly removed valid mentions (FP) | 1 |
| Remaining supported mentions | 718 |
| Remaining hallucinations (FN) | 5 |
| **Mention retention rate** | **98.1%** |

전체 mention의 98.1% 가 유지되었고, 제거된 14건 중 13건이 실제 GT 에 부재한 hallucination 이었다. 즉 *많은 mention 을 일괄 삭제하여 rate 를 낮춘 것이 아니라* 비교적 높은 precision (0.929) 으로 unsupported mention 을 선택적으로 제거한 결과이다.
...
- **본 연구 (OD-based)**: **91.34 / 90.83 / 89.97**

본 OD-based verification baseline 은 위 LVLM 들의 reported accuracy 보다 높다. 다만 이는 *generative VLM 의 yes/no 응답* 과는 다른 detector 기반 판정 방식이므로, 직접적인 모델 우열 비교라기보다는 *visual evidence 기반 검증의 가능성을 보여주는 참고 결과* 로 해석해야 한다. Precision 이 매우 높다 (95~99%) — OD 는 보수적이라 false positive 를 거의 만들지 않는다.

세 split 에서 recall 이 83.65% 로 동일하게 나타난 것은 positive object set 이 split 간에 공유되었기 때문이다 (POPE 의 random/popular/adversarial 은 동일한 yes-question 집합 + 다른 no-question 만 사용). 따라서 이를 일반적인 16% 누락률로 단정하기보다는, 본 POPE 설정에서 Florence-2 OD 가 보수적인 positive 판정을 수행했다는 증거로 해석한다.
...
| --- | --: | --: | --: | --: | --: | --: | --- |
| **Self** (Florence-2 caption + Florence-2 OD) | 290 | 2.41 | 1.05 | 1.00 | 0.571 | 0.727 | 4 / 3 |
| **Cross** (BLIP-large caption + Florence-2 OD) | 298 | 4.70 | 2.07 | 1.00 | 0.571 | 0.727 | 8 / 6 |

BLIP-large 가 Florence-2 보다 본 200장에서 약 2× 자주 hallucinate 한다 (baseline 4.70 vs 2.41). 두 설정 모두 검증 recall 이 0.571 로 관찰되었는데, 표본이 작아 (hallucination n = 7 / 14) 우연일 가능성을 완전히 배제할 수는 없다. 그럼에도 본 실험 범위에서는 captioner 가 달라져도 검증 recall 이 크게 개선되지 않음을 시사하며, **OD recall 이 주요 병목일 가능성** 을 제시한다. 같은 방향의 신호가 POPE recall 83.65% 에서도 관찰된다.
...
aggressive 매핑은 baseline rate 를 약 80% 부풀리지만 (2.46 → 4.48) F1 은 거의 변화 없다. 추가된 50 mention 의 대다수가 `table`, `baseball`, `monitor`, `plant`, `stove`, `glove` 같은 generic-to-specific 매핑에서 만들어진다. 본 결과는 object hallucination 평가에서 어휘 정규화 정책이 결과에 큰 영향을 줄 수 있음을 보여준다. 따라서 generic-to-specific 매핑을 포함한 aggressive vocab 을 사용할 경우 hallucination rate 가 과대평가될 수 있다.
...
1. **strict vocab + 500장 실험**에서 검증이 hallucination 을 통계적으로 유의하게 감소시켰다 (2.46 → 0.70%, CI 겹침 없음, F1 0.812). 동시에 mention retention 98.1% 로 over-deletion 이 아님을 확인.
2. **POPE 객체 존재 판정 변환 실험**에서 본 OD-based baseline 의 정확도가 일부 LVLM 의 reported 값보다 높다. 다만 task setting 이 달라 직접적인 모델 우열로 해석하지 않는다.
3. **Cross-model 비교** (BLIP-large caption + Florence-2 OD) 에서 검증 recall 이 captioner 변경에 robust 함이 관찰되어, 본 실험 범위에서 *OD recall 이 주요 병목일 가능성* 을 시사한다. 표본 크기 (hallucination n ≤ 14) 한계로 단정적 결론은 보류.
...
6. **표본 크기**: 본 실험의 cross-model 표본 (200 imgs) 과 baseline hallucination 수 (n ≤ 18) 가 작음. 더 큰 표본에서의 재검증이 필요.
...
본 연구는 Florence-2 의 통합 multi-task 능력을 활용한 caption-detection consistency 검증 파이프라인을 구현하고 그 효과와 한계를 실험적으로 분석했다. COCO val2017 500 장 strict vocab 실험에서 hallucination rate 가 2.46% 에서 0.70% 로 통계적으로 유의하게 감소했으며 (F1 = 0.812, mention retention 98.1%), POPE 질문을 객체 존재 판정으로 변환한 외부 baseline 에서 89.97~91.34% 정확도를 보였다. Synonym 매핑 ablation 을 통해 어휘 정책이 hallucination 측정에 큰 영향을 줄 수 있음을 정량적으로 보였고, cross-model 비교에서는 captioner 변경에도 검증 recall 이 robust 하여 본 실험 범위에서 OD recall 이 주요 병목임을 시사했다. 단일 통합 VLM 기반 self-verification 의 실용성과 동시에 그 한계의 위치를 함께 보고하는 것이 본 연구의 기여이다.
