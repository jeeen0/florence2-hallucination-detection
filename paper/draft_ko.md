...
3. COCO val2017 1000장 실험 (1433 mentions, 95% bootstrap CI 포함) 에서 baseline 2.30% → verified 0.78% 의 통계적으로 유의한 감소, F1 = 0.688. 동일 설정 500장 × 3 seed 안정성 검증에서 F1 = 0.76~0.81 범위.
...
### 4.1 데이터셋
- **주 실험**: COCO val2017 (5,000장) 에서 시드 7로 무작위 추출한 **1000장** (`instances_val2017.json` 전체 ID 풀에서 샘플링).
...
- **Cross-model**: 주 실험 1000장 풀의 앞쪽 500장.
- **Ablation**: 같은 1000장 풀의 앞쪽 500장에 대해 어휘 정책·prompt·검증 전략 변경.
...
### 5.1 주 결과 — 1000장 strict CAPTION

**표 1. Florence-2-large-ft, COCO val2017 1000장 (seed=7), strict vocab, `<CAPTION>` prompt 주 실험 결과** (95% bootstrap CI)

| Method | Mentions | Baseline Hall% | Verified Hall% | F1 (Unsupp) | Grd Acc@0.5 (n=1391) |
| --- | --: | --: | --: | --: | --: |
| `<CAPTION>` baseline | 1433 | 2.30 [1.54, 3.07] | – | – | – |
| **Ours (verified)** | **1402** | – | **0.78 [0.36, 1.28]** | **0.688 [0.526, 0.805]** | **95.69** |

검증을 통해 hallucination rate 가 통계적으로 유의하게 감소 (CI 겹침 없음). Confusion matrix: TP=22, FP=9, FN=11, TN=1391 → Precision=0.710, Recall=0.667.

`<CAPTION>` 은 짧고 보수적인 설명을 생성하기 때문에 baseline hallucination rate 가 2.30% 로 낮다. 그러나 5.2 절에서 보듯 `<DETAILED_CAPTION>` 에서는 mention 수가 약 50% 증가하고 hallucination rate 도 6.07% 로 상승하여, caption 이 자세해질수록 object hallucination 문제가 더 뚜렷하게 나타난다.
...
| --- | --: |
| Total mentions | 1433 |
| Removed as unsupported | 31 |
| Correctly removed (TP, hallucination) | 22 |
| Incorrectly removed valid mentions (FP) | 9 |
| Remaining supported mentions | 1402 |
| Remaining hallucinations (FN) | 11 |
| **Mention retention rate** | **97.8%** |

전체 mention 의 97.8% 가 유지되었고, 제거된 31건 중 22건이 실제 GT 에 부재한 hallucination 이었다 (precision 0.710). 즉 *많은 mention 을 일괄 삭제하여 rate 를 낮춘 것이 아니라* unsupported mention 을 선택적으로 제거한 결과이다.
...
500장 규모에서는 F1 이 0.76~0.81 범위로 시드에 따라 변동한다. 1000장 결과 (F1 0.688) 가 다소 낮은 것은 표본이 커지면서 OD 가 over-flag 하는 경향이 누적된 결과로 해석되며, 더 큰 표본에서 안정된 추정치 (F1 ≈ 0.69~0.79) 의 보수적 끝값에 가깝다.
