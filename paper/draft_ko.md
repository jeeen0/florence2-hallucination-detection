...
### 4.1 데이터셋
- **주 실험**: COCO val2017 (5,000장) 에서 시드 7로 무작위 추출한 **2000장** (`instances_val2017.json` 전체 ID 풀에서 샘플링).
...
- Cross-model: `Salesforce/blip-image-captioning-large` (~470M parameters); 후속 분석을 위한 `llava-hf/llava-1.5-7b-hf` (~7B parameters).
- 보조 분석: `microsoft/Florence-2-base-ft` (~230M).
- 하드웨어: 본 연구의 2000장 주 실험은 NVIDIA RTX 4090 (24 GB VRAM) 에서 수행; 500 장 보조 실험과 초기 ablation은 NVIDIA RTX 3070 (8 GB VRAM) 에서 수행. LLaVA-7B는 8 GB VRAM 환경에서는 OOM 으로 인해 사용 불가하므로 cross-model BLIP 변형만 3070에서 평가되었다.
...
500장 규모에서는 F1 이 0.76~0.81 범위로 시드에 따라 변동한다. 2000장에서 F1 이 0.690 으로 안정되는 점은 작은 표본의 F1 추정치가 점추정으로는 다소 낙관적이었음을 보여주며, 2000장 추정치 (CI [0.594, 0.770]) 가 본 시스템의 보수적이고 안정된 성능 측정치이다.
